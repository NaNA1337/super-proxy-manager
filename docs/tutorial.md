...
节点详情中的 ASN、ISP、网络类别和信誉证据来自 Core。`UNKNOWN`、`N/A` 或 `NOT EVALUATED` 表示没有完成检测，不能理解为已经通过；Manager 不再把缺失证据显示成 `BROADBAND` 或 `Passed`。Core 配置带威胁分类的信誉供应商后，VPN、公共代理、Tor 和机房标记会显示在这里，并由 Core 在晋升前硬拒绝。
...
需要立即更新节点池时，在 Slot Controller 点击 **Pull Nodes & Fill Slots**。该操作仅管理员可用，并且必须先选择具体主机。页面会显示拉取中状态以及 fetched、accepted、rejected 统计；配套 Core 会继续测试候选，直到三个活动槽位填满或本轮池耗尽。历史连接失败只保留为诊断记录，不再扣分或永久淘汰节点。
