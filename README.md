...
当前版本：[v1.0.7](https://github.com/NaNA1337/super-proxy-manager/releases/tag/v1.0.7)
...
配合 Core v1.1.10，Slot Manager 只列出已经完成 ASN、网络类型和 Reputation 审查的候选。活动槽位显示 ASN 与网络类型，手动切换窗口可以按 IP、ASN、ISP、组织和网络类型筛选；失败节点详情会显示最近一次 OpenVPN、健康检查或信誉拒绝原因。管理员可以点击 **Pull Nodes & Fill Slots** 立即拉取并审查节点，Core 会持续尝试候选直到三个槽位填满或池耗尽。
